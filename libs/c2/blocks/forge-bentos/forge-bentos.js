/**
 * forge-bentos — a Milo C2 block authored by Forge from a DISTINCTIVE Figma
 * section ("See what's in Creative Cloud Pro." — a 2×2 bento grid of
 * media+heading+body tiles on a dark card surface).
 *
 ...
 * authored class or positional index) and RECONSTRUCTS the rich bento grid with
 * createElement + classList.add, stamping its own .forge-bentos-scoped hooks
 * that the co-located forge-bentos.css keys on. Nodes (especially <picture>) are
...
export default async function init(el) {
  ...
  // RECONSTRUCT the rich layout: section-inner > [title, bento-grid > bento-row* > bento*].
  const inner = createTag('div', 'section-inner');

  if (titleEl) {
    titleEl.classList.add('title', 'title-2');
    inner.appendChild(titleEl);
  }

  const grid = createTag('div', 'bento-grid');
  // Two tiles per row (reflows to one-up under the small breakpoint via CSS).
  for (let i = 0; i < tiles.length; i += 2) {
    const row = createTag('div', 'bento-row');
    for (const tile of tiles.slice(i, i + 2)) {
