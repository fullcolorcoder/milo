/**
 * forge-use-case — a Milo C2 block authored by Forge from a DISTINCTIVE Figma
 * section ("Enhance a photo, then totally transform it." — a dark bento layout:
 * a 2-up row of media+heading+body tiles followed by a full-width media tile
 * whose caption is screen-reader-only).
 *
 ...
 * with createElement + classList.add, stamping its own .forge-use-case-scoped
 * hooks that the co-located forge-use-case.css keys on. Nodes (especially
...
// tile where the copy is visually carried by the image).
...
export default async function init(el) {
  ...
  // Two tiles per row; a trailing single tile becomes a full-width media tile
  // (its caption is carried screen-reader-only — the image speaks visually).
  for (let i = 0; i < tiles.length; i += 2) {
    const slice = tiles.slice(i, i + 2);
    if (slice.length === 1) {
      const row = createTag('div', 'uc-row uc-row-full');
      row.appendChild(buildTile(slice[0], { full: true, hiddenCap: true }));
